...
// returns all the adults
function renderFullAdultList(data) {
  $('#adult-list').empty();
  for (let i = 0; i < data.Adults.length; i += 1) {
    const adult = data.Adults[i];
    // console.log("ADULT",adult);
    // console.log("FAMILY",adult.Family.adult_type);
    const adultRow = $('<option>');
    adultRow.addClass('adult-select-option');
    // adultRow.text($(this).attr("data-adult-id"));
    adultRow.attr('value', adult.id);
    adultRow.text(`${adult.last_name}, ${adult.first_name} - ${adult.Family.adult_type}`);
    $('#adult-list').append(adultRow);
  }
}

//  listener for onclick on student form
